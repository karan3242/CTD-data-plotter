...
shiny::fluidPage(
  shiny::selectInput("fileSelect", "Select file:", choices = file_names, multiple = TRUE),
  shiny::checkboxGroupInput("varSelect", "Select variables:", choices = var, inline = TRUE)
)
```

```{r echo=FALSE}
# Shiny server logic for generating the plot
shiny::renderPlot({
  ...
  combined_data <- bind_rows(
    lapply(input$fileSelect, function(file) {
...
# Combine the Shiny UI and server to create a Shiny interactive document
shinyApp(
  ui = shiny_ui,
  server = shiny_server
)
